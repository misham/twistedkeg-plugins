---
description: Get an independent Gemini CLI review of a plan, branch, or file
model: sonnet
argument-hint: [target: file, directory, branch, or plan path]
allowed-tools: Read, Bash(gemini:*, find:*, mktemp:*, cat:*, git:*)
---
...
## Step 0: Verify Gemini CLI Installation

Before doing anything else, verify that the Gemini CLI is installed:

```bash
gemini --version 2>/dev/null
```

If this command fails (non-zero exit code or command not found):
...
Authentication is verified implicitly in Step 3. If the Gemini launch fails with an auth error, handle it gracefully there.

## Step 1: Determine the Review Target

If the user passed an argument (`$ARGUMENTS`), use that as the target. Otherwise infer from context:

| Signal | Target |
|--------|--------|
| `$ARGUMENTS` is a file/dir/branch path | That target |
...
| "review the branch" / "review the diff" | Branch diff against base branch (use `git merge-base HEAD main` to find base) |
...
**Authentication errors:** If the debug log contains OAuth, authentication, or credential errors, warn the user: "Gemini CLI is not authenticated. Run `gemini` interactively in your terminal to complete OAuth setup, then retry. Skipping Gemini review." Stop here.
