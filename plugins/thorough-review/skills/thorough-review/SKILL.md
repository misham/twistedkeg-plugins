...
If `${CLAUDE_PLUGIN_ROOT}` is not set (e.g., running outside plugin context), locate the plugin directory by searching `~/.claude/plugins/` for a directory containing a `thorough-review` plugin.
